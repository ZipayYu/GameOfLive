...
public class CellMap {
     ...
     * @param CellCol   列
     */
    public CellMap(final int CellRow, final int CellCol) {
        super();
        this.row = CellRow;
        this.col = CellCol;
    ...
    public String showGraph(final int[][] cells) {
        ...
        for (int i = 1; i < row + 1; i++) {
            ...
            str += "</tr>"; // NOPMD by Administrator on 18-10-7 下午10:42
        }
        return str;
    }
    ...
    public void setLivingCells(int[][] cells) {
        ...
        for (int index = 1; index <= num; index++) {
            int rows = random.nextInt(row) % (row - 1 + 1) + 1;
            int cols = random.nextInt(col) % (col - 1 + 1) + 1;
            cells[rows][cols] = 1;
        }
